#' ---
#' title: "Tracheid and Xylem Laser Microdissection Project, Biological QA, replication"
...
#' # Setup
#' Set the working dir
setwd("/mnt/picea/projects/spruce/akaerkoenen/laser-capture-microdissection-olga/201601/htseq")
#' ```{r set up, echo=FALSE}
#' knitr::opts_knit$set(root.dir="/mnt/picea/projects/spruce/akaerkoenen/laser-capture-microdissection-olga/201601/htseq")
#' ```
...
dir.create(file.path("analysis","201601","HTSeq"),recursive = TRUE, showWarnings = FALSE)
write.csv(count.table,"analysis/201601/HTSeq/raw-unormalised-data.csv")
...
#' The median percentage of aligned reads is 39% (25 less compared to the 64%
#' of the previous sequencing run average)
...
#' replicates. The samples however have different coverage, S4 whole sections
#' is clearly "undersampled"; but that's expected as it has an average of
#' 23.5% reads aligned.
...
#' There is some  variation, again due to the  S4 whole sections sample but
#' mothing that imparis a Variance Stabilizing Transformation (VST)
...
vst <- vst - min(vst)
write.csv(vst,"analysis/201601/HTSeq/library-size-normalized_variance-stabilized_data.csv")
...
#' deviation (sd) should be contained below 0.5.
#' 
#' The results are well in agreement with the expectations
...
#' 
#' Here we do plot the 16 technical samples, but as can be seen,
#' the technical samples are undistinguishable from one another.
#' The first 2 dimensions clearly separate the tissues by their origin,
#' which is great.
...
#' The 3rd dimension (accounting 23% of the overall variance) is what 
#' separates both whole sections replicates. This is massive but not unexpected
#' given the lower amount of aligned reads in S4 and the limited number of 
#' samples to compare (i.e. the first 3 dimensions acount for 93% of the total
#' variation among the 3 tissue type / 4 samples)
plot(pc$x[,2],
     pc$x[,3],
...
#' hence that we cabn merge them.
#' The resulting table is exported as a csv file.
m.vst <- sapply(unique(colnames(vst)),function(n,m){
  rowSums(m[,colnames(m)==n])
},vst)
write.csv(m.vst,"analysis/201601/HTSeq/library-size-normalized_variance-stabilized_merged-technical-replicates_data.csv")

#' # Conclusion
#' The raw quality of the data has limitations, but overall the expression pattern
#' appear to have a biological meaning. The S4 whole sections sample has a limited 
#' alignment depth that affect its expression profile to a certain extend. It remains, 
#' however, most similar to the S3 whole sections sample.
#' ```{r empty, eval=FALSE,echo=FALSE}
#' ```
#' 
#' # Follow up
#' 
#' I will next combine both sets of replicates and redo the same type of analysis
#' to select the samples that will be used in the Differential Expression analysis.
#' ```{r empty, eval=FALSE,echo=FALSE}
#' ```
#' # Session Info
